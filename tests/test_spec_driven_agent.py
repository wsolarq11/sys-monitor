...
async def test_execute_spec_with_cache_miss(spec_driven_agent, mock_redis):
    ...
    with (
        ...
        patch.object(
            spec_driven_agent,
            "decompose_spec",
            AsyncMock(return_value=[{"task": 1}, {"task": 2}]),
        ),
        patch.object(
            spec_driven_agent, "execute_task", AsyncMock(return_value={"status": "ok"})
        ),
        patch.object(
            spec_driven_agent,
            "reflect_on_quality",
            AsyncMock(return_value={"score": 85}),
        ),
    ):
...
async def test_quality_reflection(spec_driven_agent):
    ...
    with patch.object(
        spec_driven_agent, "reflect_on_quality", AsyncMock(return_value={"score": 75})
    ):
...
async def test_multiple_specs_execution(spec_driven_agent):
    ...
    for spec_id in spec_ids:
        with (
            ...
            patch.object(
                spec_driven_agent, "decompose_spec", AsyncMock(return_value=[])
            ),
            patch.object(
                spec_driven_agent,
                "reflect_on_quality",
                AsyncMock(return_value={"score": 80}),
            ),
        ):
