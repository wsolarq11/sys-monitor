...
async def test_e2e_complete_spec_execution_flow(
    ...
    with (
        patch.object(
            spec_driven_agent,
            "read_spec_async",
            AsyncMock(return_value={"id": spec_id}),
        ),
        patch.object(
            spec_driven_agent,
            "decompose_spec",
    ...
    with (
        ...
        patch.object(
            code_review_agent, "scan_security", AsyncMock(return_value={"issues": []})
        ),
        patch.object(
    ...
    with (
        patch.object(
            test_runner_agent,
            "run_unit_tests",
            AsyncMock(return_value={"passed": 10, "failed": 0}),
        ),
        patch.object(
            test_runner_agent,
            "run_integration_tests",
            AsyncMock(return_value={"passed": 5, "failed": 0}),
        ),
        patch.object(
            test_runner_agent,
            "run_e2e_tests",
            AsyncMock(return_value={"passed": 3, "failed": 0}),
        ),
    ):
    ...
    with (
        patch.object(
            documentation_agent, "generate_readme", AsyncMock(return_value="# README")
        ),
        patch.object(
            documentation_agent, "generate_changelog", AsyncMock(return_value="v1.0")
        ),
        patch.object(
            documentation_agent, "generate_api_docs", AsyncMock(return_value="{}")
        ),
    ):
...
async def test_e2e_parallel_agent_execution(
    code_review_agent, test_runner_agent, documentation_agent, mock_redis
):
    """
    ...
    with (
        ...
        patch.object(
            test_runner_agent,
            "run_e2e_tests",
            AsyncMock(return_value={"passed": 2, "failed": 0}),
        ),
        patch.object(
            documentation_agent, "generate_readme", AsyncMock(return_value="# README")
        ),
        ...
        print(
            f"✅ E2E-002 PASSED: Parallel execution validated ({execution_time:.3f}s)"
        )


@pytest.mark.asyncio
async def test_e2e_cache_consistency_across_agents(code_review_agent, mock_redis):
    """
...
@pytest.mark.asyncio
async def test_e2e_error_handling_and_recovery(spec_driven_agent, mock_redis):
    """
