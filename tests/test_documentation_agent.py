...
async def test_event_publishing(documentation_agent, mock_redis):
    ...
    await mock_redis.publish(
        "agent:documentation:completed", json.dumps({"docs_count": docs_count})
    )

    assert len(mock_redis.published_messages) == 1
    assert (
        mock_redis.published_messages[0]["channel"] == "agent:documentation:completed"
    )
