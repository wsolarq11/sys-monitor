...
async def test_unit_test_execution(test_runner_agent):
    ...
    mock_process.communicate = AsyncMock(
        return_value=(b'{"passed": 10, "failed": 0}', b"")
    )
...
async def test_event_publishing(test_runner_agent, mock_redis):
    ...
    await mock_redis.publish(
        "agent:test_runner:completed", json.dumps({"passed": passed, "failed": failed})
    )
...
async def test_failure_analysis(test_runner_agent):
    ...
    with patch.object(
        test_runner_agent,
        "diagnose_failure",
        AsyncMock(return_value=[{"error": "test"}]),
    ):
    ...
    for config in configs:
        with (
            patch.object(
                test_runner_agent,
                "run_unit_tests",
                AsyncMock(return_value={"passed": 10}),
            ),
            patch.object(
                test_runner_agent,
                "run_integration_tests",
                AsyncMock(return_value={"passed": 5}),
            ),
            patch.object(
                test_runner_agent,
                "run_e2e_tests",
                AsyncMock(return_value={"passed": 3}),
            ),
            patch.object(
                test_runner_agent, "analyze_failures", AsyncMock(return_value=[])
            ),
        ):
