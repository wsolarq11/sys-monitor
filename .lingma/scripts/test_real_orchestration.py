...
class EndToEndTestSuite:
    ...
    def test_spec_driven_agent_execution(self):
        ...
        try:
            ...
            assert result["status"] == "success", f"执行失败: {result.get('error_message')}"
    ...
    def test_test_runner_agent_execution(self):
        ...
        try:
            ...
            print(f"   ✓ 框架检测: {result['framework_detection']['primary_framework']}")
            print(f"   ✓ 测试执行完成")
            print(f"   ✓ 执行耗时: {duration:.2f}s")
    ...
    def test_supervisor_parallel_orchestration(self):
        ...
        try:
            ...
            start = time.time()
            result = agent.orchestrate_tasks(tasks, pattern="parallel", quality_gates_enabled=True)
            duration = time.time() - start
    ...
    def test_quality_gates_execution(self):
        ...
        try:
            ...
            for gate_name, gate_info in gates.items():
                ...
                print(f"   {status} {gate_info['name']}: {'通过' if gate_info['passed'] else '未通过'}")
    ...
    def test_decision_log_completeness(self):
        ...
        try:
            decision_log_path = self.repo_root / ".lingma" / "logs" / "decision-log.json"
            ...
            for i, entry in enumerate(recent_entries, 1):
                print(f"     {i}. [{entry.get('timestamp', 'N/A')}] {entry.get('action', 'N/A')}")
