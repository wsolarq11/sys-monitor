...
class CodeReviewerAgent:
    ...
    def calculate_quality_metrics(
        ...
        if metrics["files_analyzed"] > 0:
            metrics["complexity_score"] = round(
                total_complexity / metrics["files_analyzed"], 2
            )
        ...
        if metrics["total_lines"] > 0:
            ...
            metrics["maintainability_index"] = round(
                max(0, 100 * (1 - complexity_penalty) * (0.5 + comment_ratio)), 1
            )
    ...
    def generate_recommendations(
        self, static_analysis: Dict, metrics: Dict
    ) -> List[Dict[str, Any]]:
        """生成修复建议"""
        recommendations = []

        # 基于静态分析的建议
        if static_analysis.get("pylint") and isinstance(
            static_analysis["pylint"], list
        ):
    ...
    def generate_review_report(
        self, static_analysis: Dict, metrics: Dict, recommendations: List[Dict]
    ) -> Path:
        """生成Markdown格式的审查报告"""
...
"""
    ...
    def _calculate_overall_score(self, static_analysis: Dict, metrics: Dict) -> int:
        ...
        # 扣分项
        security_issues = static_analysis.get("security_scan", {}).get(
            "issues_found", 0
        )
    ...
    def process_request(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            report_path = self.generate_review_report(
                static_analysis, metrics, recommendations
            )
