...
class ValidationReport:
    ...
    def summary(self) -> str:
        ...
        lines.append(f"Total: {len(self.passed)} passed, {len(self.warnings)} warnings, {len(self.errors)} errors")
        lines.append("="*70 + "\n")
...
class SessionMiddleware:
    ...
    def _check_redundant_docs(self):
        ...
        if redundant:
            for doc in redundant:
                self.report.add_error(f"Redundant doc in .lingma/: {doc.name} (violates single-entry principle)")
        else:
            ...
            if subdir_path.exists():
                ...
                if readmes:
                    for readme in readmes:
                        self.report.add_error(f"README in {subdir}/ violates clean-directory principle: {readme.name}")
    ...
    def validate_root_cleanliness(self):
        ...
        forbidden_patterns = [
            '*.py', '*.ps1', '*.sh',
            '*OPTIMIZATION*', '*QUICK_REFERENCE*', '*SUMMARY*',
            '*REPORT*', '*CHECKLIST*',
            'benchmark_*', 'fix_*', 'optimize_*', 'enable_*',
            'temp_*', 'test_*.py'
        ]
    ...
    def validate_spec_state(self):
        ...
        if status not in valid_states:
            self.report.add_warning(f"Unusual spec status: '{status}' (expected one of: {', '.join(valid_states)})")
        
        # 如果状态是 draft，检查是否有未完成的澄清问题
        if status == "draft":
            content = self.spec_data.get("content", "")
            if "[NEEDS CLARIFICATION]" in content:
                self.report.add_warning("Spec is in draft state with unresolved clarification questions")
    ...
    def generate_startup_report(self, output_file: Optional[Path] = None):
        ...
        report_content = {
            ...
            "spec": {
                ...
                "status": self.spec_data["metadata"].get("status") if self.spec_data else None,
                "progress": self.spec_data["metadata"].get("progress") if self.spec_data else None,
            },
            "validation": {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Session Middleware - Validate session startup")
    parser.add_argument(
        "--lingma-dir",
        default=".lingma",
    ...
    middleware = SessionMiddleware(
        lingma_dir=args.lingma_dir,
        force_bypass=args.force_bypass
    )
