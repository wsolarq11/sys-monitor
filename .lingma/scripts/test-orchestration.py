...
def test_task_state_transitions(task_id: str):
    ...
    success = queue.update_status(
        task_id, TaskStatus.COMPLETED, result={"output": "success", "lines_added": 150}
    )
...
def test_decision_log():
    ...
    orchestrator._log_decision(
        "TEST_ACTION",
        {"test_key": "test_value", "timestamp": datetime.now().isoformat()},
    )
...
def test_priority_scheduling():
    ...
    assert (
        tasks_out == expected_order
    ), f"优先级顺序错误: {tasks_out} != {expected_order}"
...
def test_timeout_detection():
    ...
    assert (
        len(timed_out_tasks) >= 1
    ), f"未检测到超时任务（检测到{len(timed_out_tasks)}个）"
...
def test_queue_cleanup():
    ...
    try:
        ...
        cleaned_count = queue.cleanup_old_tasks(
            retention_days_completed=365, retention_days_failed=365
        )
...
def test_orchestrator_integration():
    ...
    # 提交请求
    task_id = orchestrator.submit_request("测试功能实现", priority="MEDIUM")
