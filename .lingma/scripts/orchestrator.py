...
class Orchestrator:
    ...
    def submit_request(self, user_request: str, priority: str = "MEDIUM") -> str:
        ...
        self._log_decision(
            "REQUEST_SUBMITTED",
            {"task_id": task_id, "user_request": user_request, "priority": priority},
        )

        return task_id

    def execute_and_wait(
        self, task_id: str, poll_interval: float = 2.0, timeout: int = 600
    ) -> Dict[str, Any]:
        """
        执行任务并等待完成
        ...
        while True:
            ...
            if elapsed > timeout:
                ...
                self._log_decision(
                    "TASK_TIMEOUT", {"task_id": task_id, "elapsed_seconds": elapsed}
                )
                return {
                    "status": "TIMEOUT",
            ...
            print(
                f"   {status_icon} 状态: {task.status.value} (已用时: {elapsed:.1f}s)"
            )
            ...
            if task.status == TaskStatus.COMPLETED:
                ...
                self._log_decision(
                    "TASK_COMPLETED", {"task_id": task_id, "elapsed_seconds": elapsed}
                )
...
示例用法:
  ...
  python orchestrator.py --stats
    ...
    parser.add_argument("--request", type=str, help="提交用户请求（自然语言）")
    ...
    parser.add_argument("--status", type=str, help="查询任务状态（需要提供task_id）")
    ...
    parser.add_argument(
        "--limit", type=int, default=10, help="列出任务时的最大数量（默认: 10）"
    )

    parser.add_argument("--stats", action="store_true", help="显示队列统计信息")

    parser.add_argument(
        "--repo-root", type=Path, default=None, help="仓库根目录（默认: 当前目录）"
    )
