...
class SpecTriggerVerifier:
    ...
    def check_spec_exists(self) -> bool:
        """检查1: Spec文件存在性"""
        spec_path = os.path.join(self.project_root, '.lingma', 'specs', 'current-spec.md')
    ...
    def check_validator_exists(self) -> bool:
        ...
        validator_path = os.path.join(self.project_root, '.lingma', 'scripts', 'spec-validator.py')
    ...
    def check_worker_exists(self) -> bool:
        ...
        worker_path = os.path.join(self.project_root, '.lingma', 'scripts', 'spec-worker.py')
        ...
        try:
            result = subprocess.run(
                [sys.executable, worker_path, '--help'],
                capture_output=True,
                timeout=5
            )
            return result.returncode == 0
    ...
    def check_install_script_exists(self) -> bool:
        ...
        install_path = os.path.join(self.project_root, '.lingma', 'scripts', 'install-hooks.py')
        ...
        try:
            result = subprocess.run(
                [sys.executable, install_path, '--help'],
                capture_output=True,
                timeout=5
            )
            return result.returncode == 0
    ...
    def check_clarification_detection(self) -> bool:
        ...
        # 创建临时Spec文件包含澄清问题
        temp_spec = os.path.join(self.project_root, '.lingma', 'specs', 'test-clarification.md')
...
def main():
    ...
    # 生成验证报告
    report_path = os.path.join(verifier.project_root, '.lingma', 'reports', 
