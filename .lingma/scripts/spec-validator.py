...
class SpecValidator:
    ...
    def validate(self, mode: str = 'manual') -> Dict:
        ...
        try:
            ...
            if not os.path.exists(self.spec_path):
                return self._build_result(False, mode, start_time, 
                                        ["Spec文件不存在"])
            ...
            return self._build_result(is_valid, mode, start_time, 
    ...
    def _build_result(self, is_valid: bool, mode: str, 
                     start_time: datetime, errors: List[str] = None,
                     clarifications: List = None, tasks: List = None) -> Dict:
        """构建验证结果"""
...
def main():
    ...
    try:
        ...
        if args.strict and result['warnings']:
            result['valid'] = False
            result['errors'].extend([f"警告(严格模式): {w}" for w in result['warnings']])
