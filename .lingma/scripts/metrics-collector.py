...
class MetricsCollector:
    ...
    def analyze_hooks(self) -> Dict[str, Any]:
        ...
        pre_commit_total = len(
            [l for l in pre_commit_logs if l.get("event_type") == "pre-commit-check"]
        )
        pre_commit_blocked = len(
            [l for l in pre_commit_logs if l.get("status") == "failed"]
        )

        # 统计pre-push
        pre_push_logs = hook_events.get("pre-push", [])
        pre_push_total = len(
            [l for l in pre_push_logs if l.get("event_type") == "pre-push"]
        )
        pre_push_blocked = len(
            [l for l in pre_push_logs if l.get("status") == "failed"]
        )
        ...
        return {
            "pre_commit": {
                ...
                "pass_rate": round(
                    1 - (pre_commit_blocked / max(pre_commit_total, 1)), 2
                ),
            },
            "pre_push": {
            ...
            "overall": {
                ...
                "overall_block_rate": round(
                    (pre_commit_blocked + pre_push_blocked)
                    / max(pre_commit_total + pre_push_total, 1),
                    2,
                ),
            },
        }
    ...
    def analyze_specs(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_completion_rate": (
                round(sum(completion_rates) / max(len(completion_rates), 1), 2)
                if completion_rates
                else 0
            ),
            "avg_clarification_count": (
                round(sum(clarification_counts) / max(len(clarification_counts), 1), 2)
                if clarification_counts
                else 0
            ),
            "total_clarifications": sum(clarification_counts),
            "specs_with_clarifications": len(
                [c for c in clarification_counts if c > 0]
            ),
        }
    ...
    def calculate_productivity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "commit_frequency": {
                "commits_last_7_days": len(
                    [l for l in recent_logs if l.get("hook") == "post-commit"]
                ),
                "avg_commits_per_day": round(
                    len([l for l in recent_logs if l.get("hook") == "post-commit"]) / 7,
                    2,
                ),
            },
        }
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if specs_metrics.get("avg_completion_rate", 0) < 70:
            recommendations.append("📊 Spec平均完成率低于70%，建议拆分大Spec为小任务")
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="输出文件路径（默认: stdout）"
    )
