...
class DocGeneratorAgent:
    ...
    def extract_api_docs(self, source_files: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        从源代码中提取API文档
        ...
        if not source_files:
            ...
            source_files = [str(f) for f in source_files 
                          if not any(x in str(f) for x in ['venv', '__pycache__', '.git'])][:50]
    ...
    def update_readme(self, api_docs: List[Dict], features: Optional[List[str]] = None) -> Path:
        """
        更新README.md
        ...
        if features:
            ...
            if "## Features" in readme_content:
                # 替换现有Features章节
                pattern = r'## Features\n.*?(?=\n## |\Z)'
                readme_content = re.sub(pattern, features_section, readme_content, flags=re.DOTALL)
            else:
    ...
    def generate_changelog_entry(self, changes: List[Dict[str, str]], 
                                version: str = "auto") -> Dict[str, Any]:
        """
        生成CHANGELOG条目
    ...
    def generate_documentation(self, format: str = "markdown", 
...
''', encoding='utf-8')
        ...
        assert len(api_docs) > 0
        print(f"   ✓ 提取成功: {len(api_docs)} 个模块, 类: {sum(len(d.get('classes', [])) for d in api_docs)}, 函数: {sum(len(d.get('functions', [])) for d in api_docs)}")
        ...
        # 测试4: 更新README
        print("\n✅ 测试4: 更新README")
        readme_path = agent.update_readme(api_docs, features=["New feature 1", "New feature 2"])
