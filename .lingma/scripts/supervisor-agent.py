...
class SupervisorAgent:
    ...
    def orchestrate_tasks(
        ...
        try:
            ...
            result["completed_tasks"] = sum(1 for r in task_results if r.get("status") == "success")
            result["failed_tasks"] = sum(1 for r in task_results if r.get("status") != "success")
    ...
    def _execute_conditional(self, tasks: List[Dict], quality_gates: bool) -> List[Dict]:
        """条件执行任务"""
        results = []
    ...
    def _execute_iterative(self, tasks: List[Dict], quality_gates: bool, 
    ...
    def _execute_quality_gates(self, task_results: List[Dict]) -> Dict[str, Any]:
        ...
        # Gate 3: 代码审查
        review_results = [r for r in task_results if r.get("task_type") == "code_reviewer"]
        if review_results:
            avg_score = sum(r.get("quality_score", 0) for r in review_results) / len(review_results)
            gates["gate_3_code_review"]["score"] = round(avg_score, 1)
            gates["gate_3_code_review"]["passed"] = avg_score >= 80
            gates["gate_3_code_review"]["details"].append(f"Average quality score: {avg_score:.1f}/100")
        else:
            gates["gate_3_code_review"]["score"] = 85  # 默认分数
            gates["gate_3_code_review"]["details"].append("No code review results, using default score")
        ...
        if doc_results:
            ...
            gates["gate_4_documentation"]["details"].append("No documentation generation required")
    ...
    def _make_final_decision(self, orchestration_result: Dict) -> Dict[str, Any]:
        ...
        if all_gates_passed:
            decision["reason"] = f"All quality gates passed. {completed}/{total} tasks completed successfully."
            decision["confidence"] = 0.95
        else:
            failed_gates = [
                name for name, gate in quality_gates.get("gates", {}).items()
                if not gate["passed"]
            ]
...
def run_tests():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        completed = result.get("completed_tasks", len([r for r in result.get("task_results", []) if r.get("status") == "success"]))
        total = result.get("total_tasks", len(result.get("task_results", [])))
        ...
        conditional_tasks = [
            ...
            {"task_id": "T2", "task_type": "test_runner", "condition": "env:NONEXISTENT"}
        ]
        ...
        # 测试5: 质量门禁
        print("\n✅ 测试5: 质量门禁执行")
        result = agent.orchestrate_tasks(tasks, pattern="sequential", quality_gates_enabled=True)
