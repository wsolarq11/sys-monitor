...
class PreferenceLearner:
    ...
    def _load_preferences(self) -> dict:
        ...
        return {
            ...
            "risk_threshold": {"low": 0.2, "medium": 0.5, "high": 0.8},
            "operation_overrides": {},
    ...
    def _identify_patterns(self, operations: List[dict]) -> List[dict]:
        ...
        for op_type, stats in type_stats.items():
            ...
            override_rate = (
                stats["user_overridden"] / stats["count"] if stats["count"] > 0 else 0
            )
            success_rate = (
    ...
    def _calculate_override_rate(self, operations: List[dict]) -> float:
        ...
        asked_ops = [
            op for op in operations if op.get("user_interaction", {}).get("asked")
        ]
        ...
        overridden = sum(
            1 for op in asked_ops if op.get("user_interaction", {}).get("user_decision")
        )

        return round(overridden / len(asked_ops), 2)

    def update_preferences_from_patterns(
        ...
        if updates_made:
            ...
            for update in updates_made:
                print(
                    f"   - {update['operation']}: {update['old_value']} → {update['new_value']}"
                )
        else:
            print("ℹ️ 无需调整偏好")
    ...
    def run_learning_cycle(self, days: int = 7) -> Dict[str, Any]:
        ...
        report_file = (
            self.logs_dir
            / f"learning-report-{datetime.utcnow().strftime('%Y%m%d')}.json"
        )
