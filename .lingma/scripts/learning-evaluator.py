...
class LearningEvaluator:
    ...
    def _collect_operational_metrics(self, days: int) -> Dict[str, Any]:
        ...
        success_count = sum(
            1 for op in operations if op.get("result", {}).get("status") == "success"
        )
        failed_count = sum(
            1 for op in operations if op.get("result", {}).get("status") == "failed"
        )

        auto_executed = sum(
            1 for op in operations if op.get("execution_strategy") == "auto_execute"
        )
        asked_user = sum(
            1 for op in operations if op.get("user_interaction", {}).get("asked")
        )
        overridden = sum(
            1
            for op in operations
            if op.get("user_interaction", {}).get("asked")
            and op.get("user_interaction", {}).get("user_decision")
        )

        avg_duration = (
            sum(op.get("result", {}).get("duration_ms", 0) for op in operations) / total
            if total > 0
            else 0
        )
    ...
    def _evaluate_preference_accuracy(self, days: int) -> Dict[str, Any]:
        ...
        accuracy = (
            round(correct_predictions / total_evaluated, 2)
    ...
    def _assess_system_health(self) -> Dict[str, Any]:
        ...
        health_score = (
            round(active_components / total_components, 2)
    ...
    def analyze_effectiveness(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if override_rate < 0.2:
            analysis["strengths"].append(
                f"✅ 低用户覆盖率 ({override_rate:.0%})，系统决策可靠"
            )
        ...
        if override_rate > 0.4:
            analysis["weaknesses"].append(
                f"⚠️ 用户覆盖率高 ({override_rate:.0%})，需调整策略"
            )
        if accuracy < 0.6:
        ...
        if override_rate > 0.3:
            analysis["recommendations"].append("💡 考虑提高自动化级别或降低风险阈值")
        if success_rate < 0.8:
            analysis["recommendations"].append(
                "🔧 检查高风险操作的配置，可能需要更保守的策略"
            )
        if learning_metrics.get("total_learning_cycles", 0) < 3:
            analysis["recommendations"].append("📊 建议执行更多学习周期以积累数据")
    ...
    def generate_report(self, days: int = 7, output_format: str = "text") -> str:
        ...
        report_file = (
            self.logs_dir
            / f"effectiveness-report-{datetime.utcnow().strftime('%Y%m%d')}.json"
        )
