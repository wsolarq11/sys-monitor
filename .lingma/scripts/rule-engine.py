...
class RuleEngine:
    ...
    def validate_spec(self) -> List[Violation]:
        ...
        # 记录审计日志
        self._log_audit('spec_validation', {
    ...
    def _check_automation_policy(self, rule: Rule, spec_content: str) -> List[Violation]:
        """检查自动化策略规则"""
        violations = []
...
def main():
    ...
    try:
        ...
        if args.validate_spec:
            ...
            if args.json:
                print(json.dumps([v.to_dict() for v in violations], indent=2, ensure_ascii=False))
            else:
                if violations:
