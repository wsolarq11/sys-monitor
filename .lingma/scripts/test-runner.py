...
class TestRunnerAgent:
    ...
    def run_tests(
        ...
        if framework == "auto":
            ...
            framework_info = {
                "pytest": {"command": "pytest", "coverage_flag": "--cov=. --cov-report=json"},
        ...
        try:
            ...
            # 解析结果
            test_result = self._parse_test_output(result.stdout, result.stderr, framework)
    ...
    def _generate_report(self, test_result: Dict[str, Any], format: str = "json") -> Path:
        """生成测试报告"""
...
""", encoding='utf-8')
        ...
        print(f"   ✓ 测试执行完成: {result.get('passed', 0)} 通过, {result.get('failed', 0)} 失败")
