...
impl From<std::io::Error> for AppError {
    fn from(error: std::io::Error) -> Self {
        match error.kind() {
            ...
            std::io::ErrorKind::AlreadyExists => AppError::FileSystem(format!("路径已存在：{}", error)),
        ...
        match &error {
            rusqlite::Error::SqliteFailure(sqlite_err, _) => {
                match sqlite_err.extended_code {
                    // 数据库文件相关错误
                    1007 | 1008 | 1009 => AppError::FileSystem(format!("数据库文件错误：{}", error)),
                    // 权限相关错误
            ...
            rusqlite::Error::Utf8Error(_) => AppError::FileSystem(format!("数据库编码错误：{}", error)),
            rusqlite::Error::NulError(_) => AppError::InvalidParameter(format!("数据库参数包含空字节：{}", error)),
            rusqlite::Error::InvalidParameterName(_) => AppError::InvalidParameter(format!("无效参数名：{}", error)),
            _ => AppError::Database(error.to_string()),
        }
    }
}
            ...
            RecoveryStrategy::Retry { max_attempts, delay_ms } => {
                for attempt in 1..=*max_attempts {
                    match operation() {
                        ...
                        Err(error) => {
                            crate::logger::log_warn(&format!("第 {} 次尝试失败：{}", attempt, error));
