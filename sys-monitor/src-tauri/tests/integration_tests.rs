...
fn test_database_operations() {
    ...
    let scan_id = repo.create_folder_scan("/test/path", scan_timestamp)
    ...
    // Test scan retrieval
    let scans = repo.get_folder_scans("/test/path", 10)
...
fn test_scan_small_folder() {
    ...
    let db_path = format!("{}/test.db", temp_dir.path().display());
    let repo = sys_monitor_lib::db::repository::DatabaseRepository::new(&db_path).expect("Failed to create repository");
    
    let scan_timestamp = chrono::Utc::now().timestamp();
    let scan_id = repo.create_folder_scan(scan_path, scan_timestamp).expect("Failed to create folder scan");
    
    // Simulate scanning files
    let files = vec![
        (format!("{}/file1.txt", scan_path), "file1.txt", 14u64),
        (format!("{}/file2.txt", scan_path), "file2.txt", 14u64),
        (format!("{}/subfolder/file3.txt", scan_path), "file3.txt", 14u64),
    ];
    ...
    for (path, name, size) in files {
        ...
        repo.insert_folder_item(&item).expect("Failed to insert folder item");
        total_size += size;
    }
    
    // Update scan results
    repo.update_folder_scan(scan_id, total_size, 3, 1, 0).expect("Failed to update folder scan");
    
    // Verify results
    let scans = repo.get_folder_scans(scan_path, 10).expect("Failed to get scans");
    assert_eq!(scans.len(), 1, "Should have 1 scan");
...
fn test_scan_empty_folder() {
    ...
    repo.update_folder_scan(scan_id, 0, 0, 0, 0).expect("Failed to update folder scan");
...
fn test_scan_history() {
    ...
    for i in 0..5 {
        ...
        let scan_id = repo.create_folder_scan(scan_path, scan_timestamp).expect("Failed to create folder scan");
        repo.update_folder_scan(scan_id, (100 * (i + 1)) as u64, (i + 1) as u64, 1, 0).expect("Failed to update folder scan");
    }
    
    // Test limit
    let scans = repo.get_folder_scans(scan_path, 3).expect("Failed to get scans");
...
fn test_delete_folder_scan() {
    ...
    // Delete scan
    repo.delete_folder_scan(scan_id).expect("Failed to delete folder scan");
...
fn test_file_type_distribution() {
    ...
    let stats_to_insert = vec![
        ("Documents", 3u64, 300u64),
        ("Images", 1u64, 100u64),
    ];
    ...
    for (file_type, count, total_size) in stats_to_insert {
        ...
        repo.insert_file_type_stat(&stat).expect("Failed to insert file type stat");
    }
    
    // Get file type stats
    let stats = repo.get_file_type_stats(scan_id).expect("Failed to get file type stats");
