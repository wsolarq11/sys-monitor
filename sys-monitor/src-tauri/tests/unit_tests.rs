...
mod folder_service_tests {
    ...
    use sys_monitor_lib::models::folder::{FileTypeStat, FolderItem};
    use sys_monitor_lib::utils::file_types::{
        classify_file, get_extension, get_file_type_category,
    };
    ...
    fn test_folder_scan_basic() {
        ...
        let repo = DatabaseRepository::new(&db_path).expect("Failed to create repository");
    ...
    fn test_get_extension_edge_cases() {
        ...
        assert_eq!(
            get_extension("C:\\Users\\file.doc"),
            Some("doc".to_string())
        );
    ...
    fn test_scan_error_handling_special_characters() {
        ...
        let scan_timestamp = chrono::Utc::now().timestamp();
        let scan_id = repo
            .create_folder_scan(special_path, scan_timestamp)
    ...
    fn test_batch_insert_folder_items() {
        ...
        let mut repo = DatabaseRepository::new(&db_path).expect("Failed to create repository");

        // 创建扫描记录
        let scan_timestamp = chrono::Utc::now().timestamp();
        let scan_id = repo
            .create_folder_scan("/test/batch", scan_timestamp)
            .expect("Failed to create folder scan");
        ...
        // 验证插入结果
        let retrieved_items = repo
            .get_folder_items(scan_id)
    ...
    fn test_batch_insert_file_type_stats() {
        ...
        let scan_timestamp = chrono::Utc::now().timestamp();
        let scan_id = repo
            .create_folder_scan("/test/stats", scan_timestamp)
            .expect("Failed to create folder scan");
    ...
    fn test_atomic_scan_creation() {
        ...
        // 验证扫描记录
        let scans = repo
            .get_folder_scans("/test/atomic", 10)
            .expect("Failed to get scans");
        assert_eq!(scans.len(), 1, "Should have 1 scan");
        assert_eq!(scans[0].file_count, 1, "Should have 1 file");

        // 验证 items
        let retrieved_items = repo.get_folder_items(scan_id).expect("Failed to get items");
        assert_eq!(retrieved_items.len(), 1, "Should have 1 item");

        // 验证 stats
        let retrieved_stats = repo
            .get_file_type_stats(scan_id)
...
mod system_service_tests {
    use super::*;
    use sys_monitor_lib::db::repository::DatabaseRepository;
    use sys_monitor_lib::models::metrics::{
        CpuCoreMetric, DiskMetric, NetworkMetric, SystemMetric,
    };
    ...
    fn test_disk_metric_insertion() {
        ...
        assert_eq!(
            disks[0].total_bytes, 500_000_000_000,
    ...
    fn test_concurrent_metric_insertions() {
        ...
        // 验证数据库中有 10 条记录
        let metrics = repo.get_system_metrics(100).expect("Failed to get metrics");
...
mod store_tests {
    ...
    fn test_folder_scan_state_lifecycle() {
        ...
        let scan_timestamp = chrono::Utc::now().timestamp();
        let scan_id = repo
            .create_folder_scan("/test/lifecycle", scan_timestamp)
            .expect("Failed to create scan");

        // 验证初始状态
        let scans = repo
            .get_folder_scans("/test/lifecycle", 10)
            .expect("Failed to get scans");
        assert_eq!(scans.len(), 1, "Should have 1 scan");
    ...
    fn test_scan_deletion_state_transition() {
        ...
        let scan_timestamp = chrono::Utc::now().timestamp();
        let scan_id = repo
            .create_folder_scan("/test/delete", scan_timestamp)
            .expect("Failed to create scan");

        // 验证存在
        let scans = repo
            .get_folder_scans("/test/delete", 10)
            .expect("Failed to get scans");
        assert_eq!(scans.len(), 1, "Should have 1 scan");

        // 删除扫描
        let deleted = repo
            .delete_folder_scan(scan_id)
    ...
    fn test_multiple_scans_state_isolation() {
        ...
        for path in &paths {
            let scan_id = repo
                .create_folder_scan(path, chrono::Utc::now().timestamp())
                .expect("Failed to create scan");
    ...
    fn test_scan_history_limit() {
        ...
        for i in 0..20 {
            ...
            let scan_id = repo
                .create_folder_scan("/test/history", scan_timestamp)
                .expect("Failed to create scan");
        ...
        // 测试不同限制
        let limit_5 = repo
            .get_folder_scans("/test/history", 5)
            .expect("Failed to get scans");
        assert_eq!(limit_5.len(), 5, "Should return 5 scans");

        let limit_10 = repo
            .get_folder_scans("/test/history", 10)
            .expect("Failed to get scans");
        assert_eq!(limit_10.len(), 10, "Should return 10 scans");

        // 验证返回的是最新的记录（按时间倒序）
        assert!(
            limit_10[0].scan_timestamp > limit_10[9].scan_timestamp,
    ...
    fn test_scan_timestamp_ordering() {
        ...
        // 获取所有扫描
        let scans = repo
            .get_folder_scans("/test/order", 10)
            .expect("Failed to get scans");
        ...
        for i in 0..scans.len() - 1 {
            assert!(
                scans[i].scan_timestamp >= scans[i + 1].scan_timestamp,
