...
- Name: tauri-app.spec.ts >> Tauri Application E2E Tests >> 应能导航到文件夹分析页面
- Location: tests\tauri-app.spec.ts:69:7
...
Call log:
  - waiting for locator('a[href="/folder-analysis"]')
...
```ts
  ...
  4   | 
  5   | // 应用程序路径
  6   | const APP_PATH = join(__dirname, '..', '..', '..', 'src-tauri', 'target', 'release', 'sys-monitor.exe');
  7   | 
  8   | // 测试配置
  9   | const TEST_TIMEOUT = 60000; // 60秒超时
  10  | 
  11  | test.describe('Tauri Application E2E Tests', () => {
  12  |   let appProcess: ChildProcess;
  13  | 
